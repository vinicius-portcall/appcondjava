...
- `sip_service.dart` — encapsula o pacote `sip_ua` (`SIPUAHelper`) como um `ChangeNotifier`. Controla o estado de registro SIP, o estado da chamada ativa, os streams de mídia WebRTC (local/remoto) e o envio de DTMF. Atualmente fixo em `TransportType.TCP` (veja o comentário no arquivo sobre como migrar para WSS se o Asterisk passar a usar PJSIP). Também integra a UI nativa de chamada via `flutter_callkit_incoming` — chamadas SIP entrantes disparam `FlutterCallkitIncoming.showCallkitIncoming`, e os eventos de aceitar/recusar/timeout do CallKit chamam de volta `atender()`/`desligar()`.
- `foreground_service.dart` — encapsula o `flutter_foreground_task` para manter o registro SIP ativo por meio de uma notificação persistente no Android quando o app está em segundo plano, evitando que o sistema derrube a conexão.
