  ...
  void _close() async {
    ...
    if (_localMediaStream != null && _localMediaStreamLocallyGenerated) {
      ...
      try {
        ...
        // O plugin nativo do flutter_webrtc pode lançar
        // ConcurrentModificationException aqui (visto em produção) quando o
        // dispose da stream local corre em paralelo com outro callback nativo
        // (ex: removeVideoCapturer) mexendo na mesma lista internamente. Sem
        // esse catch, essa exceção não tratada aborta o resto de close() —
        // inclusive _ua.destroyRTCSession(this) logo abaixo, o que deixa a
        // sessão "presa" pro UA mesmo depois do SIP BYE já ter sido trocado.
        logger.e(
            'close() | error disposing local MediaStream: ${error.toString()}');
      }
